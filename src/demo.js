...
class InteriorDesignApp extends xb.Script {
  ...
  async captureAndRegenerateImage() {
    ...
    try {
      ...
      if (!this.imageData) {
        throw new Error(
          "No current image. Please generate a furniture image first."
        );
      }
      ...
      if (!this.blackPainter) {
        console.warn(
          "Painter not enabled, will capture current scene directly"
        );
      }
  ...
  async generateImage(furniture = "bookshelf") {
    ...
    try {
      ...
      if (this.imageData) {
        ...
        console.log(
          "Tip: You can ask Gemini to enable the painter to modify the design"
        );
      } else {
        console.error("Gemini did not return an image");
      }
  ...
  async generateMesh() {
    ...
    try {
      ...
      this.previewPanel.setMeshProgress(0);
      for (let i = 0; i < 5; i++) {
        await new Promise((resolve) => setTimeout(resolve, 1000));
        this.previewPanel.setMeshProgress((i + 1) * 20);
      }

      await this.loadGeneratedModel(MESHY_TEST_MODEL);
